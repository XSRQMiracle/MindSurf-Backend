    ...
    async def transcribe(pcm: bytes, sample_rate: int) -> tuple[str, str | None]:
        if not recogniser_available:
            raise ConfigurationError(
                ...
                "carry: install the 'omni-asr' extra"
            )
...
def _build_generator(settings: Settings) -> Any:
    ...
    if not _importable("torch"):
        raise ConfigurationError(
            "MINDSURF_THINKER is set but torch is not installed; sync the project's "
            "base dependencies in its virtual environment"
        )
...
def _build_native(settings: Settings) -> SpeechEngine:
    ...
    if not _importable("torch"):
        raise ConfigurationError(
            "the native path needs torch; sync the project's base dependencies in its "
            "virtual environment"
        )
