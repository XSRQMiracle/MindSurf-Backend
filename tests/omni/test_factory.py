...
def test_a_missing_recogniser_package_is_503_not_500(tmp_path: Path) -> None:
    ...
    factory._importable = lambda module: module not in absent and real(module)
    try:
        engine = build(_ready(tmp_path))
    finally:
...
def test_the_wired_synthesiser_is_named_in_the_component_list(
    ...
    real = factory._importable
    monkeypatch.setattr(factory, "_importable", lambda module: module == "edge_tts" or real(module))
...
def test_the_local_synthesiser_names_its_own_extra_when_absent(tmp_path: Path) -> None:
    """The local extra must not accidentally install the hosted synthesiser."""
    for name in ("tokenizer", "SenseVoiceSmall", "mimi", "campplus"):
        (tmp_path / name).mkdir(exist_ok=True)
    settings = Settings.from_environment(
        {
    ...
    try:
        with pytest.raises(ConfigurationError, match="omni-tts-local"):
            build(settings)
    finally:
        factory._importable = real
...
def test_the_native_path_refuses_a_checkout_it_cannot_find(tmp_path: Path) -> None:
    ...
    real = factory._importable
    factory._importable = lambda module: module == "torch" or real(module)
    try:
